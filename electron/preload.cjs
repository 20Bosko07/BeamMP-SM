...
contextBridge.exposeInMainWorld('beammpApi', {
  ...
  importServerConfig: (configPath, options) => ipcRenderer.invoke('servers:importConfig', configPath, options),
  applyModsToAllServers: (sourceServerId) => ipcRenderer.invoke('servers:mods:applyToAll', sourceServerId),
  pickDirectory: () => ipcRenderer.invoke('dialog:pickDirectory'),
  pickExecutable: () => ipcRenderer.invoke('dialog:pickExecutable'),
  pickServerConfig: () => ipcRenderer.invoke('dialog:pickServerConfig'),
